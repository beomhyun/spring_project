		...
		<div id='app'>
			<div class="inner">
				...
				<div class='search_box'>
					<a href='#' @click='showlist(0)'>Q&A</a> |
					<c:if test="${not empty sessionScope.login}">
						<a href='#' @click='showlist(2)'>글쓰기</a>|
					</c:if>
				</div>
			</div>
...
<div v-if="cboard.board_code == cboard.board_groupcode">
   ...
   <input type="submit" class='blue' value='답변달기'/></form>
		...
		<script type="text/x-template" id="QnAaddtemplate">
<div class="inner">
<div>
<form action="" method="post" id="_frmForm" name="frmForm" @submit.prevent="addQnaBoard(${sessionScope.login.user_code})">
...
<button type="submit" name="button">Q&A 작성</button></td>
</tr>
</table>
</form>
</div>
...
<div class="inner">
<div>
<form action="" method="post" id="_frmForm" name="frmForm" @submit.prevent="updateQnaBoard()">
<table class="content_table">
<colgroup>
      <col style="width:30%;" />
      <col style="width:70%;" />                     
...
var QnAupdate = Vue.component('QnAupdate',{
    ...
    mounted () {
        axios
        .get('http://localhost:8197/ssafyvue/api/detailQnaBoard/' + App.boardid)
        .then(response =>
     ...
     methods: {
       updateQnaBoard() {
        ...
        axios.post('http://localhost:8197/ssafyvue/api/updateQnaBoard', {
...
var QnAadd = Vue.component('QnAadd',{
    template: '#QnAaddtemplate',
    data () {
       return {
         loading: true,
         errored: false,
...
//     	   alert("App"+App.groupid);
        ...
        axios.post('http://localhost:8197/ssafyvue/api/addQnaBoard', {
           board_title: this.ctitle,
           user_code: code,
           board_content: this.ccontent,
...
//         	 alert(this.cboard.board_code);
         ...
         update:function(bid){
             App.boardid=bid;
             App.currentview = 'QnAupdate';
             App.showlist(4);
           },
           delete_board:function(boardid){
               App.boardid=boardid;
               axios
                 .post('http://localhost:8197/ssafyvue/api/deleteQnaBoard/'+App.boardid)
                 .then(function(response){
                    location.href='./goqna.do';
                   })
                 .catch(error => {
...
var QnAlist = Vue.component('QnAlist',{
    ...
    data(){
      ...
      methods:{
         search:function(){
            axios
              .post('http://localhost:8197/ssafyvue/api/searchQnaBoards',{
         ...
         show_detail:function(bid){
            App.boardid=bid;
            axios
              .get('http://localhost:8197/ssafyvue/api/readcount/'+bid)
              //.get('./emp.json')
              .then(response => (this.info = response.data))
              .catch(error => {
                console.log(error)
      ...
      mounted () {
        axios
          .get('http://localhost:8197/ssafyvue/api/findAllQnaBoards')
          //.get('./emp.json')
          .then(response => (this.info = response.data))
          .catch(error => {
            console.log(error)
...
var App=new Vue({
  ...
  data: {
     ...
     currentview: 'QnAlist',
      boardid :'',
      allviews:['QnAlist','detailboard','QnAadd','namehrm','QnAupdate'],
      cutt:['Q&A','detail','add','search','QnAupdate'],
      groupid:''
      },
   components: {
      QnAlist: QnAlist,
